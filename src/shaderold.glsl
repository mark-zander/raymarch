...
// ray marching, returns depth and n
bool ray_marching( vec3 o, vec3 dir, inout float depth, inout vec3 n ) {
	float t = 0.0;      // total distance from camera
    float d = 10000.0;  // distance from last point
    float dt = 0.0;
    for ( int i = 0; i < 128; i++ ) {
        vec3 v = o + dir * t;   // current point
        d = dist_field( v );    // find closest object
        if ( d < 0.001 ) {      // we've gotten within epsilon of an object
            break;
        }
        dt = min( abs(d), 0.1 );    // limit step size of d
        t += dt;                    // new total dist
        if ( t > depth ) {          // we are beyond the image depth
            break;
        }
    }
    
    if ( d >= 0.001 ) {         // beyond the image depth
        return false;
    }
    
    t -= dt;                    // step back
    for ( int i = 0; i < 4; i++ ) {
        dt *= 0.5;              // make smaller step
        
        vec3 v = o + dir * ( t + dt );
        if ( dist_field( v ) >= 0.001 ) {   // approach epsilon
            t += dt;            // step closer to epsilon
        }
    }
    
    depth = t;                  // return t as depth
